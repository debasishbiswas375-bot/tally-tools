...
with st.sidebar:
    ...
    3. **Upload Excel:** Bank statement file.
    4. **Generate:** Download XML.
    """)
    st.markdown("---")
    st.info("💡 Supports: SBI, PNB, ICICI, Axis, HDFC, Kotak, Yes Bank")

# HEADER WITH LOGO (Smart Background Removal)
col_logo, col_title = st.columns([1, 4])
with col_logo:
    try:
        logo_img = Image.open('logo.png')
        # Smart logic: if image is RGBA, stick with it. If RGB, just show it.
        # This prevents crashes if the image format is simple.
