...
def extract_masters(html_file):
    ...
    # Sort by length descending to prevent 'Saha' matching 'Sahanagar'
...
def generate_tally_xml(rows, bank_led, masters, upi_fix="Suspense"):
    ...
    for row in rows:
        try:
            # CLEANING: BOB PDF data can have \n inside values
            raw_dt = str(row[2]).replace('\n', ' ').strip().split(' ')[0]
            dt = pd.to_datetime(raw_dt, dayfirst=True).strftime('%Y%m%d')
            
            nar = str(row[4]).replace('\n', ' ').replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').strip()
            mode = str(row[5]).strip().upper()
            
            # Amount handling for 3,06,140.00 format
            amt_raw = str(row[6]).replace(',', '').replace('\n', '').strip()
            amt = float(amt_raw)
    ...
    if s_file and m_file:
        ...
        with pdfplumber.open(s_file) as pdf:
            for page in pdf.pages:
                ...
                if table:
                    # Filter: BOB data lines always have a number in the first column
                    for r in table:
                        if len(r) > 6 and r[0] and any(char.isdigit() for char in str(r[0])):
                            all_rows.append(r)
        ...
        if all_rows:
            ...
            if st.button("🚀 Process & Generate Tally XML"):
                ...
                if len(xml_result) > 1000:
                    ...
                    st.error("XML result is empty. Check PDF data mapping.")
        else:
