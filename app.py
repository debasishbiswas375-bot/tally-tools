...
# --- PDF LOADER (New Feature!) ---
def load_bank_pdf(file):
    """Extracts tables from PDF and converts to DataFrame"""
    try:
        all_rows = []
        with pdfplumber.open(file) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    for row in table:
                        # Clean up row (remove None)
                        clean_row = [str(item).strip() if item else "" for item in row]
                        # Only add rows that have data
                        if any(clean_row):
                            all_rows.append(clean_row)
        
        if all_rows:
            # We assume the first row with "Date" is the header
            header_idx = 0
...
def normalize_bank_data(df, bank_name):
    ...
    if bank_name in mappings:
        ...
        # Rename columns (case insensitive search could be added here for PDF robustness)
...
with st.sidebar:
    ...
    3. **Upload File:** Excel OR PDF Bank statement.
    4. **Generate:** Download XML.
    """)
    st.markdown("---")
    st.info("💡 Supports: SBI, PNB, ICICI, Axis, HDFC, Kotak, Yes Bank")
...
with col_title:
    ...
    st.markdown('<p class="sub-header">Universal Bank to Tally XML Converter (PDF & Excel)</p>', unsafe_allow_html=True)
...
with st.container(border=True):
    st.markdown("### 📂 Step 2: Upload Bank Statement")
    ...
    with col4:
        # UPDATED: Now accepts PDF as well
        uploaded_file = st.file_uploader("Upload File (Excel or PDF)", type=['xlsx', 'xls', 'pdf'])
    ...
    if uploaded_file:
        ...
        if uploaded_file.name.endswith('.pdf'):
            with st.spinner("Extracting data from PDF..."):
                df_raw = load_bank_pdf(uploaded_file)
                if df_raw is None:
                    st.warning("Could not extract tables from this PDF. Ensure it is a text-based PDF (not scanned image).")
        else:
        ...
        if df_raw is not None:
            ...
            if st.button("✨ Generate Tally XML ✨", type="primary"):
                ...
                st.balloons()
                st.success("XML Generated Successfully! Click below to download.")
