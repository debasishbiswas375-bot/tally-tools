    ...
    if bank_file and master:
        ...
        if df is not None:
            ...
            if bank_led_choice == "⭐ AI Auto-Trace":
                # Scan first 10 rows for bank name
                sample_text = " ".join(df.head(10).astype(str).values.flatten())
                detected_bank, _ = trace_ledger_priority(sample_text, synced)
                if detected_bank != "Suspense" and detected_bank != "Untraced":
                    active_bank = detected_bank
                    st.info(f"🏦 **AI Auto-Trace Detected Bank:** {active_bank}")
                else:
                    st.warning("⚠️ AI could not detect bank. Please select manually.")
                    active_bank = st.selectbox("Manually select detected bank:", synced)
            ...
            if len(unmatched_upi_indices) > 5:
                ...
                if st.button("🚀 Process & Generate XML"):
                    ...
                    # (XML logic using active_bank and chosen_led)
...
st.markdown(f"""<div class="footer"><p>Sponsored By {s_html} <span class="brand-link">Uday Mondal</span> | Created by <span class="brand-link">Debasish Biswas</span></p></div>""", unsafe_allow_html=True)
