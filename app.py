...
def generate_tally_xml(df, bank_ledger, default_party, master_list):
    ...
    for _, row in df.iterrows():
        try:
            ...
            narration = str(row.get('Narration', '')).replace("&", "&amp;").replace("<", "&lt;")

            xml_body += f"""<TALLYMESSAGE xmlns:UDF="TallyUDF">
                <VOUCHER VCHTYPE="{vch_type}" ACTION="Create" OBJVIEW="Accounting Voucher View">
                <DATE>{date_str}</DATE><NARRATION>{narration}</NARRATION><VOUCHERTYPENAME>{vch_type}</VOUCHERTYPENAME>
...
c_html = f'<img src="data:image/png;base64,{c_logo}" width="20" style="vertical-align:middle;">' if c_logo else ""
