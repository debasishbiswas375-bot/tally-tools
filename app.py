...
def trace_identity_power(narration, master_list):
    """Deep Trace: Prevents Mithu Sk vs Mondal errors."""
...
with c2:
    ...
    if bank_file and master:
        df, meta_rows = load_data(bank_file)
        if df is not None:
            # --- AUTO-BANK DETECTION LOGIC ---
            active_bank = bank_choice
            if bank_choice == "⭐ Auto-Select Bank":
                # Convert first 50 rows of original file to string to find the bank
                full_text = " ".join(meta_rows.astype(str).values.flatten()).upper()
                if any(k in full_text for k in ["BOB", "BARODA", "138", "NASIM MIYA"]):
            ...
            if len(unmatched_upi) > 5:
                st.markdown(f'<div class="warning-box">⚠️ Found {len(unmatched_upi)} Not Traced UPI entries.</div>', unsafe_allow_html=True)
