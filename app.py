...
def trace_ledger_priority(narration, master_list):
    ...
    if "UPI" in nar_up: return "Untraced", "UPI Alert"
...
# --- 4. UI DASHBOARD ---
st.markdown('<div class="hero-container"><h1>Accounting Expert</h1><p>Data Preview & AI Trace Engine</p></div>', unsafe_allow_html=True)
...
with c2:
    ...
    if bank_file and master:
        ...
        if df is not None:
            ...
            meta = " ".join(df.head(20).astype(str).values.flatten()).upper()
            ...
            unmatched_upi = [idx for idx, row in df.iterrows() if trace_ledger_priority(row[n_c], synced)[1] == "UPI Alert"]
            
            if len(unmatched_upi) > 5:
                st.markdown(f'<div class="warning-box">⚠️ Found {len(unmatched_upi)} untraced UPI entries. Please assign a ledger:</div>', unsafe_allow_html=True)
                upi_fix = st.selectbox("Assign UPI to:", synced)
                
                # DATA PREVIEW SECTION
                st.markdown("### 📋 Data Preview (First 10 Rows)")
                preview_rows = []
                for idx, row in df.head(10).iterrows():
                    target, status = trace_ledger_priority(row[n_c], synced)
                    final_led = upi_fix if status == "UPI Alert" else target
                    preview_rows.append({"Narration": str(row[n_c])[:60], "Tally Ledger": final_led, "Match": status})
                st.table(preview_rows)
                
                if st.button("🚀 Finalize & Create Tally XML"):
                    st.success("XML Data Generated!")
                    st.download_button("⬇️ Download XML", "XML_CONTENT", file_name="tally_import.xml")
            else:
                st.markdown("### 📋 Data Preview")
                preview_rows = [{"Narration": str(row[n_c])[:60], "Tally Ledger": trace_ledger_priority(row[n_c], synced)[0], "Match": trace_ledger_priority(row[n_c], synced)[1]} for _, row in df.head(10).iterrows()]
                st.table(preview_rows)
