...
def trace_identity_power(narration, master_list):
    ...
    for ledger in master_list:
        # Uses word boundaries \b for strict identification
...
def generate_tally_xml(df, bank_led, synced, upi_fix_led=None):
    """STRICT TALLY PRIME IMPORT LOGIC"""
...
st.markdown(f'<div class="hero-container">{l_top_h}<h1>Accounting Expert</h1><p>Import-Safe Tally Engine</p></div>', unsafe_allow_html=True)
...
with c2:
    ...
    if bank_file and master_file:
        ...
        if df is not None:
            ...
            if len(unmatched) > 5:
                st.markdown(f'<div class="warning-box">⚠️ Action Required: {len(unmatched)} Untraced Items Found!</div>', unsafe_allow_html=True)
